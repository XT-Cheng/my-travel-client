...
export class DataFlushService {
  ...
  private _stateRestored$: BehaviorSubject<boolean> = new BehaviorSubject<
    boolean
  >(null);
  ...
  private createFlushEpic(): Epic<DirtyAction, DirtyAction, IAppState> {
    return (action$, store$) =>
      action$.ofType(DirtyActionTypeEnum.FLUSH).pipe(
        filter(
          action => action.payload.phaseType === DirtyActionPhaseEnum.TRIGGER,
        ),
        switchMap(action =>
          this.requestFlush(store$.value).pipe(
            mergeMap(
              (value: {
                ...
                return of(
                  dirtyRemoveAction(value.entityType)(value.id, value.type),
                ).pipe(
  ...
  private getDirtyIds(store: NgRedux<IAppState>): Observable<any> {
    return store
      .select<any>([STORE_KEY.dirties, STORE_DIRTIES_KEY.dirtyIds])
      .pipe(
        map(data => {
    ...
    Object.keys(state.dirties.dirtyIds).forEach(key => {
      Object.keys(state.dirties.dirtyIds[key]).forEach(dirtyType => {
        state.dirties.dirtyIds[key][dirtyType].forEach(id => {
          ret.push({
            entityType: getEntityType(key),
            type: DirtyTypeEnum[dirtyType.toUpperCase()],
            id: id,
          });
        });
      });
